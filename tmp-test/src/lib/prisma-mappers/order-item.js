...
exports.toPrismaOrderItem = toPrismaOrderItem;
/**
 ...
 */
function toPrismaOrderItem(item) {
    var prismaItem = {
        id: item.id,
        type: item.type,
        name: item.name,
    ...
    if (item.recipeId)
        prismaItem.recipeId = item.recipeId;
    if (item.packId)
        prismaItem.packId = item.packId;
    if (item.ingredientId)
        prismaItem.ingredientId = item.ingredientId;
