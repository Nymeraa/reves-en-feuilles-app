...
Test Packaging 2,Packaging,150,0.20,Sachet`;
  await testImport('Packaging', packagingCsv, async () => {
    ...
    const match = items.filter(
      (i) => i.name.startsWith('Test Packaging') && i.category === 'Packaging'
    );
...
Test Accessory 2,20,12.50`;
  await testImport('Accessoires', accessoryCsv, async () => {
    const items = await InventoryService.getIngredients(ORG_ID);
    const match = items.filter(
      (i) => i.name.startsWith('Test Accessory') && i.category === 'Accessoire'
    );
...
async function testImport(type: string, csvContent: string, verifyFn: () => Promise<boolean>) {
  try {
    ...
    const entity = entityMap[type] || type.toLowerCase();

    console.log(`Importing ${type} (${entity})...`);
    const result = await ImportService.executeImport(ORG_ID, entity, csvContent, {
      dryRun: false,
      upsert: true,
    });

    if (result.errors.length > 0) {
      console.error(`Import had errors for ${type}:`, JSON.stringify(result.errors, null, 2));
    }
