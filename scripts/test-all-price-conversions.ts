...
async function testAllPriceConversions() {
  ...
  console.log(
    `   Match: ${Math.abs(ingredient.weightedAverageCost - 0.025) < 0.000001 ? '✅' : '❌'}\n`
  );
  ...
  const priceInput = 35.0; // User enters €/kg
  ...
  console.log(`   Price stored: ${movement.unitPrice?.toFixed(6)} €/g`);
  console.log(`   Expected: 0.035000 €/g`);
  console.log(`   Display: ${(movement.unitPrice! * 1000).toFixed(2)} €/kg`);
  console.log(
    `   Match: ${Math.abs((movement.unitPrice || 0) - 0.035) < 0.000001 ? '✅' : '❌'}\n`
  );
  ...
  console.log(
    `   Match: ${Math.abs(updated.weightedAverageCost - expectedWAC) < 0.000001 ? '✅' : '❌'}\n`
  );

  // Summary
  const allPass =
    Math.abs(ingredient.weightedAverageCost - 0.025) < 0.000001 &&
    Math.abs((movement.unitPrice || 0) - 0.035) < 0.000001 &&
