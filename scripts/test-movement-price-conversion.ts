...
async function testMovementPriceConversion() {
  ...
  const ingredient = await InventoryService.createIngredient(orgId, {
    ...
    initialCost: 10.0, // €/kg (will be converted)
    alertThreshold: 100,
  });
  ...
  console.log(
    `   Match: ${Math.abs(ingredient.weightedAverageCost - 0.01) < 0.000001 ? '✓' : '✗ FAIL'}\n`
  );
  ...
  const movement = await InventoryService.addMovement(
    ...
    500, // 500g
    30.0 / 1000, // MANUALLY convert (simulating what the action should do)
    'Test Purchase @ 30€/kg',
    'INGREDIENT' as any,
    'MANUAL' as any
  );
  ...
  console.log(`   unitPrice: ${movement.unitPrice?.toFixed(6)} €/g`);
  ...
  console.log(
    `   Match: ${Math.abs(updatedIng.weightedAverageCost - expectedWAC) < 0.000001 ? '✓' : '✗ FAIL'}\n`
  );
