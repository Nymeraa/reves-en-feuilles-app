...
async function verify() {
    ...
    const ing = await InventoryService.createIngredient(ORG, { name: 'Audit Ing', initialStock: 100, initialCost: 5 });
    await InventoryService.addMovement(ORG, ing.id, MovementType.LOSS, -10, undefined, 'Test Loss', EntityType.INGREDIENT, MovementSource.MANUAL);

    // 2. RECIPE VERSIONING
    console.log('[2] Recipe Version');
    const recipe = await RecipeService.createRecipe(ORG, { name: 'Audit Recipe', status: RecipeStatus.ACTIVE, description: 'Test' });
    ...
    await OrderService.addItemToOrder(ORG, order.id, { type: 'RECIPE', recipeId: recipe.id, format: 100, quantity: 5 });
    ...
    await BackupService.createBackupWithLog();

    // --- VERIFICATION ---
    console.log('--- CHECKING LOGS ---');
    const logsEnd = await AuditService.getLogs();
    const newLogs = logsEnd.slice(0, logsEnd.length - countStart); // recent first usually? No audit service sorts desc.
    // Wait, AuditService sorts DESC. So new logs are at the TOP.
    // Let's grab all logs created AFTER start time? Or just grab top N.
    // We performed ~10 actions?

    // Let's filter by Timestamp roughly, or just check if we find our Entities.

    const findLog = (action: AuditAction, entity: AuditEntity, id?: string) => {
        return logsEnd.find(l => l.action === action && l.entity === entity && (!id || l.entityId === id));
    };

    const checks = [
        { name: 'Stock Movement Create', passed: !!findLog(AuditAction.CREATE, AuditEntity.STOCK_MOVEMENT) },
        { name: 'Recipe Create', passed: !!findLog(AuditAction.CREATE, AuditEntity.RECIPE, recipe.id) },
        { name: 'Recipe Version Create', passed: !!findLog(AuditAction.CREATE, AuditEntity.RECIPE_VERSION) },
        ...
        { name: 'Import Start', passed: !!logsEnd.find(l => l.action === AuditAction.IMPORT && l.metadata?.status === 'STARTED') },
        { name: 'Import Complete', passed: !!logsEnd.find(l => l.action === AuditAction.IMPORT && l.metadata?.status === 'COMPLETED') },
