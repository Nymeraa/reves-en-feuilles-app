        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="text-xs text-muted-foreground">{row.original.recipes.length} produits + {row.original.packaging.length} accessoires</div>
                </div>
            </div>
        ),
    },
        ...
        cell: ({ row }) => (
            <div className="flex flex-wrap gap-1 max-w-md">
                {row.original.recipes.slice(0, 2).map((item, idx) => {
                    ...
                    return (
                        <Badge key={idx} variant="outline" className="text-xs bg-purple-50 border-purple-100 text-purple-700 font-normal">
                            {r?.name} {item.format}g
                        </Badge>
                    )
                })}
                {row.original.recipes.length > 2 && <span className="text-xs text-muted-foreground">...</span>}
            </div>
        )
    },
    {
        ...
        header: 'STATUT',
        cell: ({ row }) => (
            <Badge className={row.original.status === 'ACTIVE' ? 'bg-purple-100 text-purple-700 hover:bg-purple-100' : 'bg-slate-100 text-slate-700 hover:bg-slate-100'}>
                {row.original.status}
            </Badge>
        )
    },
