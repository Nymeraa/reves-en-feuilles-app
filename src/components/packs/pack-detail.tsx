...
import { Trash2, Plus, Save, ArrowLeft, Package as PackageIcon, ChefHat, Calculator } from 'lucide-react'
import Link from 'next/link'
import { savePackFullAction } from '@/actions/pack'
...
export function PackDetail({ initialPack, recipes, ingredients }: PackDetailProps) {
    ...
    const handleSave = async () => {
        ...
        formData.append('data', JSON.stringify(pack));

        const res = await savePackFullAction(pack.id, null, formData);
        if (res.success) {
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <Badge variant={pack.status === 'ACTIVE' ? 'default' : 'secondary'}>{pack.status}</Badge>
                    </div>
                </div>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            {pack.recipes.map((item, idx) => (
                                <div key={item.id} className="flex items-center gap-4 p-3 border rounded bg-slate-50">
                            ...
                            {pack.recipes.length === 0 && (
                                <div className="text-center py-6 text-slate-400 text-sm">Aucune recette dans ce pack.</div>
                            )}
                        </CardContent>
                    </Card>
                                            ...
                                            >
                                                ...
                                                <SelectContent>
                                                    {packagingIngredients.map(i => (
                                                        <SelectItem key={i.id} value={i.id}>{i.name} ({i.weightedAverageCost}€)</SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                        </div>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="pt-4 space-y-4">
                                ...
                                <div className="p-3 bg-white rounded border flex justify-between items-center">
                                    ...
                                    <Badge variant={margin < 30 ? 'destructive' : 'default'} className={margin >= 30 ? 'bg-emerald-600' : ''}>
                                        {margin.toFixed(1)} %
                                    </Badge>
                                </div>
                            </div>
