...
export function PackDialog({
  ...
  // Handlers
  const addRecipe = () =>
    setPackRecipes([...packRecipes, { recipeId: '', quantity: 1, format: 50 }]);
  ...
  const addPackaging = () =>
    setPackPackaging([...packPackaging, { ingredientId: '', quantity: 1 }]);
  ...
  const removePackaging = (idx: number) =>
    setPackPackaging(packPackaging.filter((_, i) => i !== idx));
  ...
  const calculateRecipeCost = (recipeId: string, format: number) => {
    ...
    const laborCost = recipe.laborCost || 0.5;
    const stickerCost = 0.1;
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-card p-4 rounded-lg border border-purple-100 dark:border-purple-900 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select
                    value={status}
                    onValueChange={(v) => setStatus(v as PackStatus)}
                    disabled={readonly}
                  >
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    className="border-purple-200 dark:border-purple-800 font-bold"
                    disabled={readonly}
                  />
                </div>
              </div>
            ...
            <div className="bg-card p-4 rounded-lg border border-purple-100 dark:border-purple-900 shadow-sm space-y-4">
              ...
              {packRecipes.map((item, idx) => (
                <div
                  key={idx}
                  className="flex gap-2 items-center p-2 bg-purple-50/50 dark:bg-purple-900/10 rounded"
                >
                  <Select
                    value={item.recipeId}
                    onValueChange={(v) => updateRecipe(idx, 'recipeId', v)}
                    disabled={readonly}
                  >
                  ...
                  <Select
                    ...
                    onValueChange={(v) => updateRecipe(idx, 'format', parseInt(v))}
                    disabled={readonly}
                  >
              ...
              {packPackaging.map((item, idx) => (
                ...
                >
                  <Select
                    ...
                    onValueChange={(v) => updatePackaging(idx, 'ingredientId', v)}
                    disabled={readonly}
                  >
                    <SelectTrigger className="flex-1 border-purple-200 dark:border-purple-800">
            ...
            <DialogFooter>
              ...
              {!readonly && (
                ...
                >
                  {isLoading ? 'En cours...' : isEdit ? 'Enregistrer' : 'Créer le pack'}
                </Button>
              )}
            </DialogFooter>
          </form>
