...
export function OrderDetailView({ order, recipes, ingredients, packs, settings }: OrderDetailViewProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-lg border shadow-sm">
                <div className="flex items-center gap-4">
                    <div className="text-sm text-slate-500">
                        N° Commande: <span className="font-mono font-medium text-slate-900">{order.orderNumber || order.id}</span>
                    </div>
                ...
                <div className="flex gap-2">
                    ...
                    <Button variant="destructive" size="sm" onClick={async () => {
                        if (confirm('Êtes-vous sûr de vouloir supprimer cette commande ? Cela annulera les mouvements de stock associés.')) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card>
                    ...
                    <CardContent className="text-sm space-y-2">
                        <div><span className="text-slate-500 block">Nom:</span> {order.customerName}</div>
                        <div><span className="text-slate-500 block">Email:</span> {order.email || '-'}</div>
                        <div><span className="text-slate-500 block">Source:</span> {order.source || '-'}</div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent className="text-sm space-y-2">
                        <div><span className="text-slate-500 block">Transporteur:</span> {order.shippingCarrier || '-'}</div>
                        <div><span className="text-slate-500 block">N° Suivi:</span> {order.trackingNumber || '-'}</div>
                        <div><span className="text-slate-500 block">Coût (Payé par nous):</span> {order.shippingCost?.toFixed(2)} €</div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="text-sm space-y-2">
                        ...
                        <div className="border-t pt-2 mt-2">
                            <div className="flex justify-between">
                                ...
                                <span className={order.margin >= 0 ? "text-emerald-600 font-bold" : "text-red-500 font-bold"}>
                                    {order.margin?.toFixed(1)}%
                                </span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500">Profit:</span>
                                <span className={order.netProfit >= 0 ? "text-emerald-600 font-bold" : "text-red-500 font-bold"}>
            ...
            {/* Items Table */}
            <Card>
                <CardHeader><CardTitle className="text-base">Lignes de commande</CardTitle></CardHeader>
                <CardContent>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 border-b">
                                <tr>
                                    ...
                                    <th className="p-3 text-right font-medium text-slate-500">Coût Unitaire (Est.)</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                {order.items.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="p-8 text-center text-slate-400">Aucun produit</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                    <CardHeader><CardTitle className="text-base">Détail des Coûts (COGS)</CardTitle></CardHeader>
                    <CardContent className="text-sm space-y-2">
                        <div className="flex justify-between">
                ...
                <Card>
                    <CardHeader><CardTitle className="text-base">Frais & Taxes (Estimés)</CardTitle></CardHeader>
                    <CardContent className="text-sm space-y-2">
                        <div className="flex justify-between">
