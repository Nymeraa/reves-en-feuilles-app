...
export const columns: ColumnDef<Order>[] = [
  {
    ...
    header: 'N° commande',
    cell: ({ row }) => (
      <div className="font-mono text-xs">{row.original.orderNumber || row.original.id}</div>
    ),
  },
  {
  ...
  {
    accessorKey: 'parcelWeightGrams',
    header: 'Poids (kg)',
    cell: ({ row }) => {
      const grams = row.original.parcelWeightGrams;
      if (!grams) return <div className="text-muted-foreground text-xs">-</div>;
      return <div className="text-xs">{(grams / 1000).toFixed(3)}</div>;
    },
  },
  {
    accessorKey: 'email',
