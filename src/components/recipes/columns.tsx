        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="text-xs text-muted-foreground">{row.original.items.length} ingrédients</div>
                </div>
            </div>
        ),
    },
        ...
        cell: ({ row }) => (
            <div className="flex flex-wrap gap-1 max-w-md">
                {row.original.items.slice(0, 3).map((item, idx) => (
                    <Badge key={idx} variant="outline" className="text-xs bg-muted font-normal text-muted-foreground">
                        ...
                        {ingredients.find(i => i.id === item.ingredientId)?.name || 'Ingrédient'} {item.percentage}%
                    </Badge>
                ))}
                {row.original.items.length > 3 && <Badge variant="outline" className="text-xs">+ {row.original.items.length - 3}</Badge>}
            </div>
        )
    },
    {
        ...
        header: 'STATUT',
        cell: ({ row }) => (
            <Badge className={row.original.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-muted text-muted-foreground hover:bg-muted'}>
                {row.original.status}
            </Badge>
        )
    },
