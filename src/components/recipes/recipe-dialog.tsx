...
import { saveRecipeFullAction, createRecipeAction } from '@/actions/recipe'
...
export function RecipeDialog({ recipe, ingredients, trigger, open, onOpenChange, readonly = false }: RecipeDialogProps) {
    ...
    const [saveState, saveAction] = useActionState(async (prev: any, formData: FormData) => {
        ...
        const fullData = {
            ...
            description,
            status,
            items: items.filter(i => i.ingredientId && i.percentage).map(i => ({
        ...
        if (isEdit) {
            return await saveRecipeFullAction(recipe.id, prev, formData);
        } else {
            // Create mode logic: We use createRecipeAction but it only takes name.
            // We need to extend createRecipeAction or call create then update.
            // For now, let's assume we REQUIRE creating via the small button first? 
            // No, UX is better if one modal.
            // I'll simulate it: Create header -> then Update.
            const res = await createRecipeAction(prev, formData);
            if (res.recipeId) {
                return await saveRecipeFullAction(res.recipeId, prev, formData);
            }
    ...
    const calculateBatchCost = (format: number) => {
        ...
        const { ingredient: doypack, warning: doypackWarning } = findMatchingPackaging(ingredients, format, 'Sachet');
        ...
        const laborCost = recipe?.laborCost || 0.50; // Example Default
        const stickerCost = 0.10; // Example
            ...
            */}
            <Dialog open={effectiveOpen} onOpenChange={setEffectiveOpen}>
                <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto bg-muted/30 dark:bg-background">
                    <DialogHeader>
                        <DialogTitle>
                            {readonly ? 'Détail de la recette' : (isEdit ? 'Modifier la recette' : 'Nouvelle recette')}
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <form action={async (fd) => { await saveAction(fd); setEffectiveOpen(false); }} className="space-y-6">
                        ...
                        <div className="bg-card p-4 rounded-lg border shadow-sm space-y-4">
                            <div className="grid gap-2">
                                <Label>Nom *</Label>
                                <Input value={name} onChange={e => setName(e.target.value)} name="name" required disabled={readonly} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                    <Label>Statut</Label>
                                    <Select value={status} onValueChange={(v) => setStatus(v as RecipeStatus)} disabled={readonly}>
                        ...
                        <div className="bg-card p-4 rounded-lg border shadow-sm space-y-4">
                            ...
                            {items.length === 0 && <p className="text-sm text-muted-foreground italic">Ajoutez des ingrédients...</p>}

                            {items.map((item, idx) => (
                                <div key={idx} className="flex gap-2 items-center">
                                    <Select value={item.ingredientId} onValueChange={v => updateItem(idx, 'ingredientId', v)} disabled={readonly}>
                                    ...
                                    return (
                                        <div key={f} className="text-center p-2 bg-muted/50 rounded border">
                                            ...
                                            <div className="font-bold text-foreground">
                                                {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(costs.total)}
                                            </div>
                                    ...
                                    return (
                                        <div key={f} className="flex flex-col gap-2 p-3 border rounded-md bg-muted/30">
                                            <div className="flex justify-between items-center border-b pb-2">
                                                ...
                                                <span className="text-xs text-muted-foreground">Coût: {cost.toFixed(2)}€</span>
                                            </div>

                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    <Label className="text-[10px] text-muted-foreground uppercase">Prix Vente</Label>
                                                    <div className="relative">
                                                        <Input
                                                            type="number"
                                                            step="0.01"
                                                            value={prices[f] || ''}
                                                            onChange={e => setPrices({ ...prices, [f]: parseFloat(e.target.value) })}
                                                            className="h-8 pl-5 text-sm"
                                                            disabled={readonly}
                                                        />
                                                        <span className="absolute left-1.5 top-2 text-xs text-muted-foreground">€</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    <Label className="text-[10px] text-muted-foreground uppercase">Marge %</Label>
                                                    <div className={`flex items-center justify-center h-8 text-sm font-bold rounded border ${margin < 30 ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-red-200 dark:border-red-900' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900'}`}>
                                                        {margin.toFixed(0)}%
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {!costs.doypackFound && <div className="text-[10px] text-red-500 text-center">⚠ Pas de Sachet</div>}
                                        </div>
                                    )
                                })}
                            </div>
