...
export function MovementTable({ movements = [], ingredients = [], onAdd }: MovementTableProps) {
  ...
  const filtered = movements.filter((m) => {
    ...
    const matchesSearch =
      ingName.includes(search.toLowerCase()) ||
            ...
            {filtered.map((m) => {
              ...
              return (
                <TableRow key={m.id}>
                  ...
                  <TableCell>
                    <div className="font-medium text-foreground">
                      {getIngredientName(m.ingredientId)}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {isPerUnit ? 'Packaging/Accessoire' : 'Ingrédient'}
                    </div>
                  </TableCell>
                  <TableCell
                    className={
                  ...
                  >
                    {m.deltaQuantity > 0 ? '+' : ''}
                    {m.deltaQuantity} {quantityUnit}
                  </TableCell>
                  <TableCell className="text-muted-foreground text-sm">
                    {/* Placeholder for "Before -> After" */}-
                  </TableCell>
                  <TableCell className="text-sm text-foreground">{priceDisplay}</TableCell>
