...
const CATEGORIES = ['Thé Vert', 'Thé Noir', 'Thé Blanc', 'Rooibos', 'Infusion', 'Fleur', 'Fruit', 'Épice', 'Plante', 'Autre'];

export function IngredientDialog({ ingredient, trigger, open, onOpenChange, readonly = false, suppliers = [] }: IngredientDialogProps) {
    ...
    return (
        <Dialog open={effectiveOpen} onOpenChange={setEffectiveOpen}>
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>
                        {readonly ? 'Détail de l\'ingrédient' : (isEdit ? 'Modifier l\'ingrédient' : 'Nouvel ingrédient')}
                    </DialogTitle>
                </DialogHeader>
                ...
                <form action={async (formData) => {
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input id="name" name="name" defaultValue={ingredient?.name} placeholder="Ex: Thé vert Sencha" required disabled={readonly} />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-2">
                            <Label>Catégorie</Label>
                            <Select name="category" defaultValue={ingredient?.category || 'Autre'} disabled={readonly}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                        ...
                        <div className="grid gap-2">
                            ...
                            <Select name="supplierId" defaultValue={ingredient?.supplierId || "NO_SUPPLIER"} disabled={readonly}>
                                <SelectTrigger>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input name="supplierUrl" defaultValue={ingredient?.supplierUrl} placeholder="https://..." disabled={readonly} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input name="alertThreshold" type="number" defaultValue={ingredient?.alertThreshold || 100} disabled={readonly} />
                        </div>
                    </div>
                    ...
                    {isEdit && (
                        <div className="grid gap-2">
                            ...
                            <div className="flex items-center gap-2">
                                <Input disabled value={(ingredient.weightedAverageCost * 1000).toFixed(2)} className="bg-muted" />
                    ...
                    <div className="grid gap-2">
                        <Label>Notes</Label>
                        <Textarea name="notes" defaultValue={ingredient?.notes} placeholder="Notes internes..." disabled={readonly} />
                    </div>
