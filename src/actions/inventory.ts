...
export async function createIngredientAction(prevStateOrData: any, formData?: FormData) {
    let name, category, supplierId, supplierUrl, stock, cost, threshold, notes, subtype, dimensions, capacity;
...
export async function updateIngredientAction(id: string, prevStateOrData: any, formData?: FormData) {
    let name, category, supplierId, supplierUrl, threshold, notes, subtype, dimensions, capacity, currentStock, weightedAverageCost;
    ...
    try {
        const updated = await InventoryService.updateIngredient('org-1', id, {
            ...
            initialCost: (weightedAverageCost !== undefined && !isNaN(weightedAverageCost)) ? weightedAverageCost : undefined
        } as any)
