    ...
    return (
        <form action={async (formData) => {
            ...
            <div className="flex items-center justify-between">
                ...
                <Button type="submit" disabled={isSaving} className="bg-emerald-600 hover:bg-emerald-700 text-white dark:bg-emerald-700 dark:hover:bg-emerald-800">
                        ...
                        <CardContent className="space-y-8">
                            <div className="grid grid-cols-4 gap-4">
                                <div className="space-y-2">
                                    <Label>URSSAF (%)</Label>
                                    <Input name="urssafRate" defaultValue={settings.urssafRate} type="number" step="0.01" />
                                </div>
                                <div className="space-y-2">
                                    <Label>Shopify (% transaction)</Label>
                                    <Input name="shopifyTransactionPercent" defaultValue={settings.shopifyTransactionPercent} type="number" step="0.01" />
                                </div>
                                <div className="space-y-2">
                                    <Label>Shopify (€ fixe)</Label>
                                    <Input name="shopifyFixedFee" defaultValue={settings.shopifyFixedFee} type="number" step="0.01" />
                                </div>
                                <div className="space-y-2">
                                    <Label>Autres frais par défaut (€)</Label>
                                    <Input name="defaultOtherFees" defaultValue={settings.defaultOtherFees} type="number" step="0.01" />
                                </div>
                            </div>
                            ...
                            <div className="bg-muted/50 border border-border rounded-lg p-4 text-sm">
                                ...
                                <div className="text-muted-foreground space-y-1">
                                    ...
                                    <ul className="list-disc pl-5 space-y-0.5 text-xs opacity-90">
                                        <li>URSSAF : 30€ × {settings.urssafRate}% = {(30 * settings.urssafRate / 100).toFixed(2)}€</li>
                                        <li>Shopify : 30€ × {settings.shopifyTransactionPercent}% + {settings.shopifyFixedFee}€ = {(30 * settings.shopifyTransactionPercent / 100 + settings.shopifyFixedFee).toFixed(2)}€</li>
                                        <li>Autres frais : {settings.defaultOtherFees}€</li>
                                        <li className="font-bold pt-1">Total frais : {(30 * settings.urssafRate / 100 + 30 * settings.shopifyTransactionPercent / 100 + settings.shopifyFixedFee + settings.defaultOtherFees).toFixed(2)}€</li>
                                    </ul>
                                </div>
                            </div>
                        </CardContent>
                        ...
                        <CardContent className="space-y-8">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <Input name="tvaIngredients" defaultValue={settings.tvaIngredients} type="number" step="0.01" />
                                </div>
                                <div className="space-y-2">
                                    <Label>TVA Packaging & Accessoires (%)</Label>
                                    <Input name="tvaPackaging" defaultValue={settings.tvaPackaging} type="number" step="0.01" />
                                </div>
                            </div>
                            ...
                            <div className="bg-muted/50 border border-border rounded-lg p-4 text-sm">
                                ...
                                <div className="text-muted-foreground space-y-1">
                                    <ul className="list-disc pl-5 space-y-0.5 text-xs opacity-90">
                                        <li>Ingrédients (thés, infusions) : TVA réduite à {settings.tvaIngredients}%</li>
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-5 gap-4">
                        ...
                        <SummaryItem label="Shopify" value={`${settings.shopifyTransactionPercent}% + ${settings.shopifyFixedFee}€`} />
