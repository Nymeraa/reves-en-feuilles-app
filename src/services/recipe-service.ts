...
export const RecipeService = {
  ...
  async getRecipeVersion(
    ...
    const found = versions.find(
      (v) => v.recipeId === recipeId && v.versionNumber === versionNumber
    );
  ...
  async updateRecipeCostsForIngredient(orgId: string, ingredientId: string, newCostPerKg: number) {
    // We need to import PackService dynamically to avoid circular dependencies if any
    const { PackService } = await import('./pack-service');

    const recipes = await db.readAll<Recipe>('recipes', orgId);
    const costMap = await this.getIngredientCostMap(orgId);
    // Note: costMap relies on the DB, so make sure the ingredient update is committed before calling this.
    ...
    for (const recipe of recipes) {
      ...
      // Optimization: Only write if cost actually changed significantly?
      // For now, write to ensure consistency.
