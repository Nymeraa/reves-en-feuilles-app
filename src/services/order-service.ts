...
export const OrderService = {
    ...
    async addItemToOrder(orgId: string, orderId: string, input: AddOrderItemInput) {
        ...
        if (input.type === 'RECIPE') {
            if (!input.recipeId || !input.format) throw new Error('Recipe ID and Format required for Recipe Item');
    ...
    async calculateFinancials(orgId: string, order: Order) {
        ...
        if (order.packagingType && order.packagingType !== 'Aucun') {
            ...
            let carton = order.packagingId ? ingredients.find(i => i.id === order.packagingId) : undefined;
        ...
        order.feesShopify = roundCurrency((totalRevenue * (settings.shopifyTransactionPercent / 100)) + settings.shopifyFixedFee);
        if (order.feesOther === undefined || order.feesOther === null) order.feesOther = 0.1;
        order.feesTotal = roundCurrency((order.feesUrssaf || 0) + (order.feesShopify || 0) + (order.feesOther || 0));

        order.netProfit = roundCurrency(totalRevenue - order.totalCost - (order.shippingCost || 0) - order.feesTotal);
    ...
    async deductOrderStock(orgId: string, order: Order) {
        for (const item of order.items) {
            if (item.type === 'RECIPE') {
                ...
                const doypackIng = await PackagingService.findPackagingForFormat(orgId, item.format || 0, 'Sachet');
                if (doypackIng) {
                    await InventoryService.addMovement(
                        orgId,
                        doypackIng.id,
                ...
                for (const pRecipe of packToUse.recipes) {
                    ...
                    const doypack = await PackagingService.findPackagingForFormat(orgId, pRecipe.format as number, 'Sachet');
                    if (doypack) {
                        await InventoryService.addMovement(
                            orgId,
                            doypack.id,
        ...
        if (order.packagingType) {
            ...
            let carton = order.packagingId ? allIngredients.find(i => i.id === order.packagingId) : undefined;
            if (!carton && order.packagingType) carton = allIngredients.find(i => i.id === order.packagingType);
            if (!carton && order.packagingType) carton = allIngredients.find(i => i.name === order.packagingType);
    ...
    async confirmOrder(orgId: string, orderId: string) {
        ...
        await ActivityService.log(orgId, 'UPDATE' as any, 'Order', order.id, `Order confirmed: ${order.totalAmount.toFixed(2)}€`);
    },

    async updateOrder(orgId: string, orderId: string, input: CreateOrderInput, itemsInput?: AddOrderItemInput[]) {
        const order = await db.getById<Order>('orders', orderId, orgId);
        if (!order) throw new Error('Order not found');
            ...
            for (const itemIn of itemsInput) {
                ...
                if (itemIn.type === 'RECIPE' && itemIn.recipeId) {
                    ...
                    if (r) {
                        ...
                        const doypack = await PackagingService.findPackagingForFormat(orgId, itemIn.format || 0, 'Sachet');
                        if (doypack) {
                            const packCostPerUnit = doypack.weightedAverageCost || 0;
                            unitPackagingCost = packCostPerUnit * tvaPackResult;
                        }
        ...
        for (const item of order.items) {
            if (item.type === 'RECIPE') {
                ...
                for (const rItem of version.items) {
                    ...
                    await InventoryService.addMovement(orgId, rItem.ingredientId, MovementType.ADJUSTMENT, usage, undefined, `Revert Order #${order.id} - Restock`, EntityType.INGREDIENT, MovementSource.ORDER, order.id);
                }

                const doypackIng = await PackagingService.findPackagingForFormat(orgId, item.format || 0, 'Sachet');
                if (doypackIng) {
                    await InventoryService.addMovement(orgId, doypackIng.id, MovementType.ADJUSTMENT, item.quantity, undefined, `Revert Order #${order.id} - Restock Doypack`, EntityType.PACKAGING, MovementSource.ORDER, order.id);
                }
            } else if (item.type === 'PACK') {
                if (!item.packId) continue;
                ...
                for (const pRecipe of version.recipes) {
                    ...
                    for (const ing of pRecipeEntity.items) {
                        ...
                        await InventoryService.addMovement(orgId, ing.ingredientId, MovementType.ADJUSTMENT, usage, undefined, `Revert Order #${order.id} (Pack) - Restock`, EntityType.INGREDIENT, MovementSource.ORDER, order.id);
                    }
                }
                for (const pPkg of version.packaging) {
                    await InventoryService.addMovement(orgId, pPkg.ingredientId, MovementType.ADJUSTMENT, pPkg.quantity * item.quantity, undefined, `Revert Order #${order.id} (Pack) - Return Pkg`, EntityType.PACKAGING, MovementSource.ORDER, order.id);
                }
            }
        }
