...
export type ImportEntityType = 'Ingrédients' | 'Packaging' | 'Accessoires' | 'Fournisseurs' | 'Recettes' | 'Packs' | 'Commandes';
...
export const ImportService = {
    ...
    async executeImport(orgId: string, type: ImportEntityType, rows: any[], mode: 'create' | 'upsert' = 'create') {
        ...
        try {
            switch (type) {
                ...
                case 'Accessoires':
                    ...
                    break;
                case 'Fournisseurs':
                    result = await SupplierService.bulkCreateSuppliers(rows);
                    break;
                case 'Recettes':
                    ...
                    for (const row of rows) {
                        const existing = mode === 'upsert' ? (await RecipeService.getRecipes(orgId)).find(r => r.name === row.name) : undefined;
                ...
                case 'Packs':
                    const packs = [];
                    for (const row of rows) {
                        const existing = mode === 'upsert' ? (await PackService.getPacks(orgId)).find(p => p.name === row.name) : undefined;
                        if (existing && mode === 'upsert') {
                ...
                case 'Commandes':
                    const orders = [];
                    for (const row of rows) {
                        const existing = mode === 'upsert' ? (await OrderService.getOrders(orgId)).find(o => o.orderNumber === row.orderNumber) : undefined;
                        if (existing && mode === 'upsert') {
