import { Ingredient, IngredientStatus, CreateIngredientInput, StockMovement, MovementType, EntityType, MovementSource } from '@/types/inventory';
...
export const InventoryService = {
    ...
    async getLowStockIngredients(orgId: string): Promise<Ingredient[]> {
        ...
        return ingredients.filter(i =>
            i.status === IngredientStatus.ACTIVE &&
            i.currentStock < (i.alertThreshold || 100)
        );
    },
    ...
    async bulkCreateIngredients(orgId: string, inputs: CreateIngredientInput[], mode: 'create' | 'upsert'): Promise<Ingredient[]> {
    ...
    async updateIngredient(orgId: string, id: string, updates: Partial<CreateIngredientInput> & { status?: IngredientStatus }): Promise<Ingredient> {
    ...
    async recomputeWAC(orgId: string, entityId: string): Promise<number> {
        const movements = await this.getMovements(orgId);
        const purchases = movements.filter(m => m.ingredientId === entityId && m.type === MovementType.PURCHASE);

        let totalQty = 0;
        let totalCost = 0;

        for (const p of purchases) {
            if (p.unitPriceAtTime !== undefined) {
                totalQty += p.deltaQuantity;
                totalCost += p.deltaQuantity * p.unitPriceAtTime;
            }
        }
