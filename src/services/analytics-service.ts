...
export const AnalyticsService = {
    async getSummary(orgId: string, startDate?: Date, endDate?: Date): Promise<AnalyticsSummary> {
        ...
        const filtered = orders.filter(o => {
            ...
            // Whitelist Status
            if (![OrderStatus.PAID, OrderStatus.SHIPPED, OrderStatus.DELIVERED].includes(o.status)) return false;
        ...
        for (const order of filtered) {
            ...
            const orderFees = order.feesTotal !== undefined ? order.feesTotal : (order.otherFees || 0);
            fees += orderFees;
        }
    ...
    async getTopProducts(orgId: string, limit = 5): Promise<TopProduct[]> {
        ...
        for (const order of orders) {
            ...
            if (![OrderStatus.PAID, OrderStatus.SHIPPED, OrderStatus.DELIVERED].includes(order.status)) continue;
    ...
    async getTopRecipes(orgId: string, limit = 10, startDate?: Date, endDate?: Date): Promise<Array<{
        id: string;
        name: string;
        quantitySold: number;
        revenue: number;
    ...
    async getRecentSales(orgId: string, limit = 5): Promise<Order[]> {
        const orders = readData<Order[]>('orders.json', []);
        return orders
            .filter(o => o.organizationId === orgId && [OrderStatus.PAID, OrderStatus.SHIPPED, OrderStatus.DELIVERED].includes(o.status))
    ...
    async getSalesTimeline(orgId: string, days = 30) {
        ...
        orders.forEach(o => {
            if (o.organizationId !== orgId || ![OrderStatus.PAID, OrderStatus.SHIPPED, OrderStatus.DELIVERED].includes(o.status)) return;
    ...
    async getOrderProfitability(orgId: string, startDate?: Date, endDate?: Date) {
        ...
        const detailedOrders = filtered.map(order => {
            ...
            const isShippingMissing = shippingCharged > 0 && (order.shippingCost === undefined || order.shippingCost === null);
            ...
            const totalFees = order.feesTotal !== undefined ? order.feesTotal : (order.otherFees || 0);
        ...
        // Global Stats
        const stats = detailedOrders.reduce((acc, o) => {
        ...
        const globalMarginPercent = stats.totalRevenue > 0 ? (stats.totalMargin / stats.totalRevenue) * 100 : 0;

        return {
            orders: detailedOrders.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
            stats: {
                ...stats,
                globalMarginPercent
            }
    ...
    async getOrderProfitabilityCsv(orgId: string, startDate?: Date, endDate?: Date) {
        ...
        const data = orders.map(o => ({
            ...
            TotalFees: o.totalFees.toFixed(2), // Added
